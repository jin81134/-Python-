...
while True:
    ...
    try:
        # 合计人数
        Ba_ba_api1 = Ba_ba_api - Bili_ba_api - Tap_ba_api
        sum = Ba_ba_api1 + Bili_ba_api + Tap_ba_api
        # 获取当前时间
        now = datetime.now()
        minute = now.strftime("%Y-%m-%d %H:%M")  # 格式化为精确到分钟的字符串

        # 定义CSV文件的字段名称
        field_names = ['时间', '官网数据', '实际官网预约人数', 'BiliBili数据', 'TapTap数据', '合计']  # 表头
        file_write = [minute, Ba_ba_api, Ba_ba_api1, Bili_ba_api, Tap_ba_api, sum]  # 写入变量数据
